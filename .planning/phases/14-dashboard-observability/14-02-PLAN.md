...
Purpose: Users currently have no visibility into application logs from the web UI -- they must check container stdout/stderr. A live-updating log viewer section on the dashboard lets users see what Triggarr is doing (search cycles, errors, connection issues) without leaving the browser.
...
**Update `triggarr/logging.py`:**

In `setup_logging`, after adding the redacting sink, also add the log_buffer sink. Import `log_buffer` from `triggarr.log_buffer`. Add the buffer sink at the SAME level as the redacting sink but with NO format string (the buffer stores raw record data, not formatted text):
...
- This keeps secret handling in logging.py where the secrets list lives.
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && uv run python -c "from triggarr.log_buffer import LogBuffer, LogEntry; b = LogBuffer(5); b.add(LogEntry('ts', 'INFO', 'test')); assert len(b.get_recent()) == 1; print('OK')"</automated>
...
<task type="auto">
  ...
  <action>
**Create `triggarr/templates/partials/log_viewer.html`:**
...
<div id="log-viewer"
  ...
  {% if not log_entries %}
  <p class="text-triggarr-muted text-sm">No log messages yet.</p>
  {% else %}
  <div class="space-y-0 max-h-64 overflow-y-auto font-mono text-xs">
    {% for entry in log_entries[:30] %}
    <div class="flex items-start gap-2 py-1 {% if not loop.last %}border-b border-triggarr-border/30{% endif %}">
        <span class="text-triggarr-muted whitespace-nowrap">{{ entry.timestamp }}</span>
...
**Update `triggarr/templates/dashboard.html`:**
...
**Update `triggarr/web/routes.py`:**

1. Add import at top: `from triggarr.log_buffer import log_buffer`
...
Note: The test_app fixture does NOT need modification for log_buffer since it's a module-level singleton. However, clear it in fixture setup to avoid test pollution: add `log_buffer.clear()` at the start of the test_app fixture, and import log_buffer.
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && uv run pytest tests/test_log_buffer.py tests/test_web.py -x -q</automated>
  </verify>
  <done>
