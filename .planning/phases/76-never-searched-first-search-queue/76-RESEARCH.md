...
| QUEUE-03 | Remove `missing_cursor`/`cutoff_cursor`; pre-upgrade `state.json` loads clean, treated as everything-unsearched (no separate migration function) | `total=False` TypedDict; dispatch reads `ist.get("<q>_searched", [])`. **CORRECTED (codex HIGH-1):** `{**default, **instance_data}` (state.py:143) PRESERVES unknown keys and `save_state` writes them back, so leftover cursor keys would persist — `_merge_defaults` therefore ACTIVELY STRIPS them on load (`merged.pop("missing_cursor"/"cutoff_cursor", None)`). Load→save round-trip test asserts they're ABSENT from written JSON. |
...
|----------|-------------|------------------|
| Stored data | `state.json` per-instance dicts carry `missing_cursor`/`cutoff_cursor` (int). NEW `missing_searched`/`cutoff_searched` start absent → read as `[]`. `missing_pass`/`cutoff_pass` are KEPT and carry forward. | **No separate migration function** (D-09/QUEUE-03). **CORRECTED (codex HIGH-1):** the original "overwritten on next save" claim was WRONG — `{**default, **instance_data}` (state.py:143) preserves unknown keys and `save_state` writes them back. `_merge_defaults` therefore ACTIVELY STRIPS the legacy cursor keys on load (`merged.pop(...)`); the load→save round-trip test asserts they are ABSENT from the written JSON. New `*_searched` read via `ist.get("<q>_searched", [])`. |
...
**The canonical question — after every file is updated, what runtime systems still have the old value?** Only an on-disk `state.json` from a pre-upgrade run, holding leftover `missing_cursor`/`cutoff_cursor` integer keys. **CORRECTED (codex HIGH-1):** these are NOT read-harmless-and-overwritten — they would persist (the merge preserves unknown keys), so `_merge_defaults` actively STRIPS them on load (`merged.pop(...)`). This is the **one** runtime-state item; the QUEUE-03 contract handles it via strip-on-load (no separate migration function) — needs a dedicated load→save back-compat test asserting the keys are gone from written JSON (Validation Architecture).
...
4. **Back-compat state load (QUEUE-03):** load a pre-upgrade `state.json` with `missing_cursor`/`cutoff_cursor` and no searched-logs → loads clean via `_merge_defaults`, dispatch treats everything unsearched. **CORRECTED (codex HIGH-1):** `_merge_defaults` actively STRIPS the legacy cursor keys on load (they are NOT left to be overwritten); the test loads→saves→reloads and asserts `missing_cursor`/`cutoff_cursor` are ABSENT from the written JSON while `missing_pass`/`missing_searched` are correct. New test in `test_state.py`.
