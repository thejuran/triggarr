...
must_haves:
  ...
  key_links:
    - from: "tests/test_db.py"
      ...
      pattern: "from triggarr.db import.*get_search_history"
...
Add import for `get_search_history` in the existing import line from `triggarr.db`.
...
All tests are async (pytest-asyncio auto mode). Follow existing patterns: `tmp_path` fixture, `await init_db(db_path)`, `await insert_search_entry(...)`.
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && uv run pytest tests/test_db.py -x -q</automated>
  </verify>
  <done>
