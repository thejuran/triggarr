...
3. **Docker job caching**: Add `docker/setup-buildx-action@v3` step, then update the `docker build` command to use `docker buildx build` with `--cache-from type=gha` and `--cache-to type=gha,mode=max` for GitHub Actions cache backend. Keep the `-t triggarr:ci-test .` tag and add `--load` so the image is available locally.
...
4. Run `uv run ruff check triggarr/ tests/` to confirm linting passes locally (same as what CI lint job runs).
...
- Linting passes locally (`uv run ruff check triggarr/ tests/`)
</verification>
