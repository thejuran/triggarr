...
| RCOV-02 | Token value not in HTTP response body or headers (RESPONSE only — token IS in the mint log by design) | T-72-redaction | No token leak in response | integration | `uv run pytest tests/test_reset.py::test_token_not_in_response -x` | ❌ W0 | ⬜ pending |
| RCOV-02 | Token value IS present in the mint warning log (deliberate operator-only recovery channel, D-02) | T-72-redaction | Token in warning log + 0600 file; NOT in response | integration | `uv run pytest tests/test_reset.py::test_token_in_mint_log -x` | ❌ W0 | ⬜ pending |
| RCOV-03 | While a live unexpired token exists, /reset/request is a no-op (no mint/TTL-reset/file-rewrite) — supersession-DoS guard (H1) | T-72-supersede-dos | Remote caller cannot invalidate operator's live token | integration | `uv run pytest tests/test_reset.py::test_live_token_request_is_noop -x` | ❌ W0 | ⬜ pending |
