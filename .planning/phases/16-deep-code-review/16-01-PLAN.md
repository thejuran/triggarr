...
3. Update the error message to say "Blocked address" (covers both link-local and loopback).
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && uv run pytest tests/test_validation.py tests/test_db.py tests/test_web.py -x -q</automated>
...
Add a test that calls `get_search_history(db_path, per_page=0)` and verifies it returns a valid result (no ZeroDivisionError). Name: `test_get_search_history_zero_per_page_defaults`.
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && uv run pytest tests/test_validation.py tests/test_web.py tests/test_db.py -x -q</automated>
  </verify>
  <done>
