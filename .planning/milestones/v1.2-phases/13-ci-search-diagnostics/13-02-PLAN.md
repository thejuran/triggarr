...
must_haves:
  truths:
    - "Triggarr logs the detected Sonarr API version (v3 or v4) at startup"
...
<task type="auto">
  ...
  <files>triggarr/clients/sonarr.py, triggarr/startup.py, tests/test_startup.py</files>
  <action>
**SonarrClient version detection (`triggarr/clients/sonarr.py`):**
...
**Startup integration (`triggarr/startup.py`):**
...
Follow existing test patterns: `pytest-asyncio` with `asyncio_mode=auto`, `MockTransport` for client-level tests, `AsyncMock` for integration-level tests.
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && uv run pytest tests/test_startup.py -x -q -k "version"</automated>
  </verify>
...
<task type="auto">
  ...
  <action>
Add diagnostic summary logging to both `run_radarr_cycle` and `run_sonarr_cycle` in `triggarr/search/engine.py`.
...
Also add `import time` at the top of engine.py alongside existing imports.
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && uv run pytest tests/test_search.py -x -q && uv run ruff check triggarr/search/engine.py</automated>
  </verify>
...
- Mock `insert_search_entry` (patch `triggarr.search.engine.insert_search_entry`)
...
Follow existing test patterns in `tests/test_search.py` and `tests/conftest.py`.
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && uv run pytest tests/test_search.py -x -q -k "diagnostic"</automated>
  </verify>
...
<verification>
- `triggarr/clients/sonarr.py` has `detect_api_version` method that returns "v3" or "v4"
- `triggarr/startup.py` calls version detection after successful Sonarr connection validation
- `triggarr/search/engine.py` logs cycle summary (duration, fetched, searched, skipped) for both apps
- All diagnostic logs at INFO level
- `uv run pytest tests/ -x -q` passes with all new tests
- `uv run ruff check triggarr/ tests/` passes
</verification>
