...
Add import for `get_search_history` from `triggarr.db`.
...
- Context should include: `result` (the dict), `active_apps` (list), `active_queues` (list), `active_outcomes` (list), `search_text` (str).
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && uv run python -c "from triggarr.db import get_search_history; from triggarr.web.routes import history_page, partial_history_results; print('imports ok')"</automated>
...
<a href="/history"
   class="{% block nav_history_class %}text-triggarr-muted{% endblock %} hover:text-white text-sm">
  History
</a>
<a href="/settings"
   class="{% block nav_settings_class %}text-triggarr-muted{% endblock %} hover:text-white text-sm">
  Settings
</a>
```
...
Create `triggarr/templates/history.html` extending base.html:
- Set title block: "Search History - Triggarr"
- Set nav blocks: nav_dashboard_class = text-triggarr-muted, nav_history_class = text-white, nav_settings_class = text-triggarr-muted
...
Create `triggarr/templates/partials/history_results.html`. This is the htmx-swappable partial. Structure:
...
- App filter: "Radarr" pill, "Sonarr" pill. Each pill is an `<a>` tag styled as a pill button.
  ...
  - Inactive (filtered out): `bg-triggarr-card text-triggarr-muted border border-triggarr-border`
- Queue type filter: "missing" pill, "cutoff" pill
  - Active: `bg-triggarr-green/20 text-triggarr-green`
  - Inactive: `bg-triggarr-card text-triggarr-muted border border-triggarr-border`
- Outcome filter: "searched" pill, "failed" pill
  - Active for searched: `bg-triggarr-green/20 text-triggarr-green`
  - Active for failed: `bg-red-500/20 text-red-400`
  - Inactive: same as queue inactive
- Text search box: `<input type="search" name="search" placeholder="Search by title..." ...>` with Tailwind dark styling matching the project theme (`bg-triggarr-bg border border-triggarr-border text-triggarr-text rounded px-3 py-1.5 text-sm`).
...
- Each entry row: same compact single-line format as dashboard search_log partial. Use the exact same styling:
  ...
  - Queue type: `<span class="text-xs text-triggarr-muted">{{ entry.queue_type }}</span>`
  - Outcome badge: `<span class="text-xs px-1.5 py-0.5 rounded {% if entry.outcome == 'failed' %}bg-red-500/20 text-red-400{% else %}bg-triggarr-green/20 text-triggarr-green{% endif %}">{{ entry.outcome or 'searched' }}</span>`
  - Timestamp: `<span class="text-xs text-triggarr-muted">{{ entry.timestamp[:19] | replace('T', ' ') }}</span>`

**Empty state:**
- If `result.entries` is empty: `<p class="text-triggarr-muted text-sm py-8 text-center">No search history yet. Searches will appear here after your first scheduled run.</p>`
...
- Active page: `bg-triggarr-green text-white` pill. Other pages: `bg-triggarr-card text-triggarr-muted border border-triggarr-border hover:text-white`.
...
env = Environment(loader=FileSystemLoader('triggarr/templates'))
...
<verification>
1. `uv run python -c "from triggarr.db import get_search_history; print('ok')"` — import succeeds
2. `uv run python -c "from triggarr.web.routes import history_page, partial_history_results; print('ok')"` — routes importable
3. Jinja2 template parsing succeeds for all 3 templates
4. `uv run ruff check triggarr/db.py triggarr/web/routes.py` — no lint errors
</verification>
