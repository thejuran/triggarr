...
<task type="auto">
  ...
  <action>
1. In `triggarr/models/config.py`, add `hard_max_per_cycle: int = 0` to `GeneralConfig`. Value 0 means unlimited (no cap). Must be >= 0.

2. In `triggarr/config.py`, add the new field to `DEFAULT_CONFIG` template under `[general]` section as a commented-out line:
   ```
   # hard_max_per_cycle = 0   # 0 = unlimited; caps total items searched per app per cycle
   ```

3. In `triggarr/search/engine.py`, add a helper function `apply_hard_max(missing_count: int, cutoff_count: int, hard_max: int) -> tuple[int, int]` that:
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && uv run python -c "from triggarr.models.config import GeneralConfig; g = GeneralConfig(); assert g.hard_max_per_cycle == 0; print('OK')" && uv run python -c "from triggarr.search.engine import cap_batch_sizes; assert cap_batch_sizes(5, 5, 0) == (5, 5); assert cap_batch_sizes(5, 5, 6) == (3, 3); assert cap_batch_sizes(5, 5, 10) == (5, 5); print('OK')"</automated>
  </verify>
...
<task type="auto">
  ...
  <action>
1. In `triggarr/web/routes.py`, in the `settings_page` GET handler, add `hard_max_per_cycle` to the template context:
   ```python
   "hard_max_per_cycle": settings.general.hard_max_per_cycle,
   ```

2. In `triggarr/web/routes.py`, in the `save_settings` POST handler, read the form field and include it in `new_config["general"]`:
   ```python
   "hard_max_per_cycle": safe_int(form.get("hard_max_per_cycle"), 0, 0, 1000),
   ```
   Note: use 0 as default (unlimited), min 0, max 1000.

3. In `triggarr/templates/settings.html`, add a field to the General section (after log_level) for hard_max_per_cycle:
   ```html
   <div>
       <label class="block text-sm text-triggarr-muted mb-1">Hard Max Items Per Cycle</label>
       ...
       <p class="text-xs text-triggarr-muted mt-1">0 = unlimited. Caps total items per app per cycle.</p>
   </div>
   ```
...
4. In `tests/test_search.py`, add tests for `cap_batch_sizes`:
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && uv run pytest tests/test_search.py -x -q -k "hard_max or cap_batch"</automated>
  </verify>
...
3. Import check: `uv run python -c "from triggarr.search.engine import cap_batch_sizes; print('OK')"`
</verification>
