...
must_haves:
  truths:
    - "Search history entries are stored in a SQLite database at /config/triggarr.db"
...
2. Create `triggarr/db.py` with the following functions:

   **DB_PATH**: `Path("/config/triggarr.db")` (same volume as config and state)
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && uv sync --extra dev && uv run python -c "import aiosqlite; print('aiosqlite OK')" && uv run python -c "from triggarr.db import init_db, insert_search_entry, get_recent_searches, migrate_from_state; print('imports OK')"</automated>
  </verify>
  <done>aiosqlite is in pyproject.toml and installed. triggarr/db.py exists with init_db, insert_search_entry, get_recent_searches, and migrate_from_state functions. All functions use aiosqlite.connect context manager pattern.</done>
</task>
...
<task type="auto">
  ...
  <action>
1. **triggarr/search/scheduler.py** -- In `create_lifespan`:
   - Import `from triggarr.db import init_db, migrate_from_state`
...
2. **triggarr/search/engine.py** -- Replace `append_search_log` with async SQLite insert:
   - Remove the `append_search_log` function entirely (it will no longer be used)
   - Remove the `SEARCH_LOG_MAX = 50` constant
   - Import `from triggarr.db import insert_search_entry`
...
3. **triggarr/search/scheduler.py** -- Update `make_search_job` to pass db_path:
...
4. **triggarr/web/routes.py** -- Update routes to read from SQLite:
   - Import `from triggarr.db import get_recent_searches`
...
5. **triggarr/state.py** -- Keep `search_log` in `TriggarrState` TypedDict for backwards compatibility (migration reads it), but it will be empty after migration. No structural changes needed, but add a comment noting that search_log is deprecated in favor of SQLite.

6. **triggarr/templates/partials/search_log.html** -- No changes needed. The template already iterates over a list of dicts with `name`, `timestamp`, `app`, `queue_type` keys, and `get_recent_searches` returns dicts with those same keys.

7. **triggarr/startup.py** -- No changes needed. Startup doesn't touch search_log.
  </action>
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && uv run ruff check triggarr/ --select E,F,I</automated>
...
2. Update `tests/test_search.py`:
   ...
   - Import `from triggarr.db import init_db` in test file.
...
4. Update `tests/conftest.py` if shared fixtures need db_path:
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && uv run pytest tests/ -x -q</automated>
  </verify>
...
3. `grep -r "append_search_log" triggarr/` returns no results (function fully removed)
4. `uv run python -c "from triggarr.db import init_db, insert_search_entry, get_recent_searches; print('OK')"` -- imports work
</verification>

<success_criteria>
- aiosqlite is in pyproject.toml dependencies
- triggarr/db.py exists with init_db, insert_search_entry, get_recent_searches, migrate_from_state
