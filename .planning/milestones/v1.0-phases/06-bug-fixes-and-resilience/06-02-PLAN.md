...
Output: Updated `triggarr/clients/base.py` and `triggarr/search/engine.py` with correct exception handling.
</objective>
...
<task type="auto">
  ...
  <files>triggarr/clients/base.py</files>
  <action>
Two changes to `triggarr/clients/base.py`:
...
Do NOT change the `get_paginated` method's exception handling -- `ValidationError` there should propagate to the cycle abort handler (the caller handles it). Do NOT change function signatures.
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && python -m pytest tests/test_clients.py -x -q</automated>
...
<task type="auto">
  ...
  <action>
Two changes to `triggarr/search/engine.py`:
...
Add `import pydantic` at the top of the file alongside the existing imports.
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && python -m pytest tests/test_search.py -x -q</automated>
...
<verification>
```bash
cd /Users/julianamacbook/triggarr && python -m pytest tests/test_clients.py tests/test_search.py -x -v
```
