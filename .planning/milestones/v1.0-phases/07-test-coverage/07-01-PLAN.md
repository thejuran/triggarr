...
2. **`default_state` helper function** (plain factory, not a fixture):
   - Simply re-exports `_default_state()` from `triggarr.state` for convenience.
   - This avoids every test file having to import from `triggarr.state` directly.

Keep the file simple -- no autouse fixtures, no MockTransport fixtures (those are test-local since each test needs a different handler). Import `Settings`, `ArrConfig` from `triggarr.models.config` and `_default_state` from `triggarr.state`.
  </action>
  <verify>
