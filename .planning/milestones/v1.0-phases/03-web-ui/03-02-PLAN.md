...
<task type="auto">
  ...
  <action>
    ...
    **Update triggarr/search/engine.py — run_radarr_cycle:**
    ...
    **Update triggarr/search/engine.py — run_sonarr_cycle:**
...
from triggarr.state import AppState
...
<task type="auto">
  ...
  <action>
    ...
    **Rewrite triggarr/templates/partials/app_card.html:**
    ...
    - Connection status indicator on the right side of the header:
      - If `app.connected` is `true` (Python True): green dot — a small `w-2.5 h-2.5 rounded-full bg-triggarr-green` span
    ...
    Label styling: `text-xs uppercase tracking-wide text-triggarr-muted`
    ...
    **Rewrite triggarr/templates/partials/search_log.html:**
    ...
    If search_log is empty: `<p class="text-triggarr-muted text-sm">No searches yet.</p>`
    ...
    The app badges use Radarr's orange and Sonarr's blue to match their ecosystem branding, while the overall Triggarr theme is green.
...
card = Path('triggarr/templates/partials/app_card.html').read_text()
...
log = Path('triggarr/templates/partials/search_log.html').read_text()
...
import inspect
from triggarr.web.routes import app_card_partial
