...
must_haves:
  truths:
    ...
    - "Lifespan exposes triggarr_state, settings, scheduler, clients, config_path, and state_path on app.state"
  ...
  key_links:
    - from: "triggarr/web/routes.py"
      to: "triggarr/search/scheduler.py"
...
<task type="auto">
  ...
  <action>
    ...
    **Create triggarr/web/__init__.py:**
    Empty package marker file.

    **Create triggarr/web/routes.py:**
    ```python
    ...
    **Rewrite triggarr/search/scheduler.py:**
...
from triggarr.web.routes import router, TEMPLATES_DIR, STATIC_DIR
from pathlib import Path
...
from triggarr.search.scheduler import create_lifespan, make_search_job
import inspect
...
<task type="auto">
  ...
  <action>
    ...
    ```
    mkdir -p triggarr/templates/partials triggarr/static/css
    ```
    ...
    **triggarr/templates/base.html:**
    ...
    - `<body>` with class `bg-triggarr-bg text-triggarr-text min-h-screen`
    - Nav bar: `bg-triggarr-card border-b border-triggarr-border`. Contains flex row with "Triggarr" brand text (text-triggarr-green font-bold text-xl) on the left, and "Dashboard" / "Settings" links on the right. Links use `text-triggarr-muted hover:text-white`. Active link detection via Jinja2 `{% block %}` overrides.
    - `<main>` container: `max-w-5xl mx-auto px-6 py-6` wrapping `{% block content %}{% endblock %}`
    - `{% block title %}Triggarr{% endblock %}` in the `<title>` tag
    ...
    **triggarr/templates/partials/app_card.html:**
    ...
    ```
    Card contents:
    - Header row: App name capitalized (text-lg font-semibold), styled with a colored left border (green = `border-l-4 border-l-triggarr-green`)
    ...
    - Each label in `text-xs uppercase text-triggarr-muted`, each value in `text-sm`
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && python -c "
from pathlib import Path
templates = Path('triggarr/templates')
...
"</automated>
  ...
  <done>All templates created with dark theme, htmx polling on partials (5s interval), nav bar with Dashboard/Settings links. Tailwind CSS compiled with custom triggarr color theme. Static files served via /static mount.</done>
</task>

</tasks>

<verification>
- `python -c "from triggarr.web.routes import router"` imports without error
- `python -c "from triggarr.search.scheduler import create_lifespan, make_search_job"` imports without error
- All 5 templates exist in triggarr/templates/
- triggarr/static/css/output.css exists (compiled Tailwind)
