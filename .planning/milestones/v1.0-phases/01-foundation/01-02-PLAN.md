...
Purpose: Provide the HTTP communication layer that connects Triggarr to *arr applications. These clients are used by the startup validation flow (Plan 03) and by the search engine (Phase 2).
...
<task type="auto">
  ...
  <action>
    1. Create `triggarr/models/arr.py` with response models:
    ...
    2. Create `triggarr/clients/base.py` with `ArrClient` base class:
...
<task type="auto">
  ...
  <action>
    1. Create `triggarr/clients/radarr.py` with `RadarrClient(ArrClient)`:
    ...
    2. Create `triggarr/clients/sonarr.py` with `SonarrClient(ArrClient)`:
    ...
    Export both clients from `triggarr/clients/__init__.py`:
    ```python
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && python -c "
from triggarr.clients import RadarrClient, SonarrClient
import inspect
...
"</automated>
  ...
  <done>
    ...
    - Both clients exported from triggarr.clients package
  </done>
</task>
...
<verification>
1. `python -c "from triggarr.clients import RadarrClient, SonarrClient; print('Clients importable')"` -- both clients importable
2. `python -c "from triggarr.models.arr import PaginatedResponse, SystemStatus; print('Models OK')"` -- response models work
