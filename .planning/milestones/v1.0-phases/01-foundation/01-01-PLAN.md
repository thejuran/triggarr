...
must_haves:
  truths:
    - "Settings load from /config/triggarr.toml with [general], [radarr], [sonarr] sections"
...
<objective>
Set up the Triggarr Python package with project scaffolding, TOML configuration loading via pydantic-settings, loguru-based logging with API key redaction, and atomic JSON state persistence.
...
<task type="auto">
  ...
  <action>
    1. Create `pyproject.toml` with project metadata:
       - name: "triggarr", version: "0.1.0", requires-python: ">=3.11"
       - Dependencies: `pydantic-settings[toml]`, `httpx`, `loguru`, `tomli-w`, `fastapi`, `uvicorn[standard]`
       - Script entry point: `triggarr = "triggarr.__main__:main"`

    2. Create `triggarr/__init__.py` with `__version__ = "0.1.0"`

    3. Create `triggarr/models/__init__.py` (empty)

    4. Create `triggarr/clients/__init__.py` (empty)

    5. Create `triggarr/models/config.py` with Pydantic models:
       ...
       - Config file path: use a module-level constant `CONFIG_PATH = Path("/config/triggarr.toml")` that can be overridden for testing
    ...
    6. Create `triggarr/config.py` with:
    ...
    Important: API keys use `SecretStr` — this is the first security invariant. `SecretStr.__repr__` and `__str__` both return `**********`, preventing accidental exposure in any string formatting, logging, or serialization.
  </action>
  <verify>
  ...
  <action>
    ...
    2. Create `triggarr/state.py`:
       ...
       - Define state structure as a TypedDict or dataclass:
         ...
         ```
       - `load_state(state_path: Path) -> TriggarrState`: Read JSON file, return parsed dict. If file doesn't exist, return default empty state with both apps at cursor 0.
       - `save_state(state: TriggarrState, state_path: Path) -> None`: Atomic write using `tempfile.NamedTemporaryFile(mode="w", dir=parent, suffix=".tmp", delete=False)` + `json.dump` + `os.fsync` + `os.replace`. Ensure parent directory exists.
       - The atomic write pattern prevents corruption if the process crashes mid-write (locked user decision).
  </action>
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && python -c "
from triggarr.logging import setup_logging, create_redaction_filter
...
from triggarr.state import load_state, save_state
...
<verification>
1. `python -c "from triggarr.models.config import Settings; print('Config OK')"` -- imports succeed
2. `python -c "from triggarr.logging import setup_logging; print('Logging OK')"` -- imports succeed
3. `python -c "from triggarr.state import load_state, save_state; print('State OK')"` -- imports succeed
...
<success_criteria>
- All triggarr package modules importable without error
