...
must_haves:
  ...
  artifacts:
    ...
    - path: "triggarr/static/js/htmx.min.js"
  ...
  key_links:
    ...
    - from: "triggarr/templates/base.html"
      to: "triggarr/static/js/htmx.min.js"
...
<task type="auto">
  ...
  <action>
Create `triggarr/web/middleware.py` with an `OriginCheckMiddleware` class inheriting from `starlette.middleware.base.BaseHTTPMiddleware`:
...
Register middleware in `triggarr/__main__.py` in the `_run()` function, AFTER creating the FastAPI app and BEFORE `app.include_router(router)`:
```python
...
Use FastAPI TestClient with a minimal test app that has the middleware and a simple POST endpoint. Follow existing test patterns (see tests/test_web.py for project conventions).
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && python -m pytest tests/test_middleware.py -x -v</automated>
  </verify>
...
- Final line becomes: `exec setpriv --reuid="$PUID" --regid="$PGID" --init-groups --no-new-privileges python -m triggarr`

**htmx vendoring (SECR-07):**

Download htmx 2.0.8 minified JS:
- Run: `curl -sL https://unpkg.com/htmx.org@2.0.8/dist/htmx.min.js -o triggarr/static/js/htmx.min.js`
- Create the `triggarr/static/js/` directory first if it doesn't exist

Update `triggarr/templates/base.html`:
- Replace `<script src="https://unpkg.com/htmx.org@2.0.8"></script>` with `<script src="{{ url_for('static', path='js/htmx.min.js') }}"></script>`
  </action>
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && grep -q "127.0.0.1:8080:8080" docker-compose.yml && grep -q "cap_drop" docker-compose.yml && grep -q "no-new-privileges" docker-compose.yml && grep -q "no-new-privileges" entrypoint.sh && test -f triggarr/static/js/htmx.min.js && grep -q "url_for" triggarr/templates/base.html && ! grep -q "unpkg" triggarr/templates/base.html && echo "ALL CHECKS PASSED"</automated>
  </verify>
...
2. `grep -c "unpkg" triggarr/templates/base.html` returns 0 (no CDN references)
...
5. `stat -f "%Lp" triggarr/static/js/htmx.min.js` confirms file exists (non-empty)
</verification>
