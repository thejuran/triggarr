...
must_haves:
  ...
  key_links:
    - from: "triggarr/web/routes.py"
      ...
      pattern: "from triggarr\\.web\\.validation import"
...
<task type="auto">
  ...
  <action>
Create `triggarr/web/validation.py` with three helper functions:
...
6. None -> "info"
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && python -m pytest tests/test_validation.py -x -v</automated>
  </verify>
...
<task type="auto">
  ...
  <action>
**Update `triggarr/web/routes.py` save_settings function:**

Add import at top of file:
```python
from triggarr.web.validation import validate_arr_url, safe_int, safe_log_level
```
...
**Update `triggarr/config.py` generate_default_config:**
...
This ensures both first-run default config and web UI saves produce files with restrictive permissions.
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && python -m pytest tests/ -x -v</automated>
  </verify>
...
3. `grep -c "safe_int" triggarr/web/routes.py` returns 3 (three integer fields clamped)
4. `grep -c "validate_arr_url" triggarr/web/routes.py` returns at least 1 (URL validation active)
5. `grep -c "0o600" triggarr/web/routes.py triggarr/config.py` returns 2 (both write paths secured)
6. `grep -c "safe_log_level" triggarr/web/routes.py` returns 1 (log level allowlist active)
</verification>
