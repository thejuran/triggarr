...
must_haves:
  truths:
    - "`docker compose up` starts Triggarr and the web UI is reachable in a browser"
  ...
  artifacts:
    ...
    - path: "docker-compose.yml"
      ...
      contains: "triggarr_config:/config"
  ...
  key_links:
    ...
    - from: "entrypoint.sh"
      ...
      via: "exec setpriv ... python -m triggarr"
...
<objective>
Create Docker packaging for Triggarr: multi-stage Dockerfile with pytailwindcss builder stage and slim production image, docker-compose.yml with named volume, PUID/PGID entrypoint, HEALTHCHECK, and localhost URL detection at startup.

Purpose: Enable self-hosters to run Triggarr as a Docker container with `docker compose up`, following *arr ecosystem conventions (/config volume, PUID/PGID, LinuxServer.io patterns).
...
Stage 1 (`builder`): Based on `python:3.13-slim`. Set `ENV TAILWINDCSS_VERSION=v4.2.1`. Copy `pyproject.toml` and `triggarr/` directory. Run `pip install --no-cache-dir pytailwindcss`, then `tailwindcss_install`, then `tailwindcss -i triggarr/static/css/input.css -o triggarr/static/css/output.css --minify`. This stage only produces the compiled CSS artifact.

Stage 2 (production): Based on `python:3.13-slim`. Set `ENV PYTHONUNBUFFERED=1 PYTHONDONTWRITEBYTECODE=1`. WORKDIR `/app`. Copy `pyproject.toml` and `triggarr/` then `pip install --no-cache-dir .` (runtime deps only, no dev extras). Copy compiled `output.css` from builder via `COPY --from=builder /build/triggarr/static/css/output.css triggarr/static/css/output.css`. Copy `entrypoint.sh` to `/entrypoint.sh` and `chmod +x`. EXPOSE 8080. VOLUME /config. Add HEALTHCHECK: `--interval=30s --timeout=5s --start-period=10s --retries=3 CMD python3 -c "import urllib.request; urllib.request.urlopen('http://localhost:8080/')" || exit 1`. Set `ENTRYPOINT ["/entrypoint.sh"]`. Do NOT use CMD — the entrypoint exec's python directly. Add a comment near TAILWINDCSS_VERSION explaining the pin.
...
3. Create group with `groupadd -g "$PGID" triggarr` if GID doesn't exist (`getent group "$PGID"`)
4. Create user with `useradd -u "$PUID" -g "$PGID" -d /config -s /sbin/nologin triggarr` if UID doesn't exist
5. `chown -R "$PUID:$PGID" /config`
6. `exec setpriv --reuid="$PUID" --regid="$PGID" --init-groups python -m triggarr`
...
- Service `triggarr` with `image: ghcr.io/thejuran/triggarr:latest`, `container_name: triggarr`
- Environment: `PUID=1000`, `PGID=1000`
- Volumes: `triggarr_config:/config`
- Ports: `"8080:8080"`
- Restart: `unless-stopped`
- Top-level `volumes:` declaring `triggarr_config:`
- Add a comment explaining users can replace `image:` with `build: .` for local builds
  </action>
  <verify>
    <automated>test -f Dockerfile && test -f entrypoint.sh && test -f .dockerignore && test -f docker-compose.yml && grep -q "FROM python:3.13-slim" Dockerfile && grep -q "setpriv" entrypoint.sh && grep -q "triggarr_config:/config" docker-compose.yml && grep -q ".venv" .dockerignore && echo "PASS" || echo "FAIL"</automated>
...
<task type="auto">
  ...
  <action>
Add a `check_localhost_urls` function to `triggarr/startup.py` that warns if any enabled app's URL points to localhost (a common Docker networking mistake).
...
Use loguru's `capfd` or a sink capture pattern. If existing test patterns use a different approach, follow those. The test should construct a Settings object with the necessary fields (use the same pattern as existing tests — check tests/ for examples).
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && python -m pytest tests/ -x -q 2>&1 | tail -5</automated>
...
5. docker-compose.yml uses named volume `triggarr_config` mapped to `/config`
