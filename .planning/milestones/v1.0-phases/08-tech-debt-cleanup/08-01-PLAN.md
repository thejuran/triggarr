...
must_haves:
  ...
  artifacts:
    - path: "triggarr/web/routes.py"
      ...
      contains: "from triggarr.models.config import Settings as SettingsModel"
  ...
  key_links:
    - from: "triggarr/templates/settings.html"
      to: "triggarr/web/routes.py:save_settings"
    ...
    - from: "tests/test_web.py:test_search_now_happy_path"
      to: "triggarr/web/routes.py:search_now"
...
<task type="auto">
  ...
  <action>
In `triggarr/web/routes.py`, delete line 22 (`from triggarr.config import load_settings`). No other code in routes.py references `load_settings` -- this import became orphaned when Phase 6 replaced the reload pattern with direct `SettingsModel(**new_config)` construction.

In `tests/test_web.py`, clean up 3 test functions that have dead `@patch("triggarr.web.routes.load_settings")` decorators:

1. `test_save_settings_writes_toml` (around line 153):
   - Delete the `@patch("triggarr.web.routes.load_settings")` decorator line
...
The tests will continue to pass because they verify TOML output via `config_path.read_text()`, which exercises the actual `save_settings` code path (SettingsModel construction + tomli_w write). The dead patches were providing false confidence that a function was being tested when it was never called.
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && python -m pytest tests/test_web.py -x -q 2>&1 | tail -5</automated>
    <manual>Confirm no import of load_settings in routes.py and no mock_load in test_web.py</manual>
  </verify>
  <done>
    - `grep -n 'load_settings' triggarr/web/routes.py` returns 0 matches
...
In `triggarr/templates/settings.html`, line 11, replace the hardcoded form action:
```html
<!-- BEFORE -->
...
The test patches `run_radarr_cycle` to return the existing mock state (so the template can render the app card), and patches `save_state` to prevent actual file I/O. The `search_lock` is already available from the fixture (Part B).
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && python -m pytest tests/test_web.py::test_search_now_happy_path tests/test_web.py::test_save_settings_writes_toml -x -q 2>&1 | tail -5</automated>
    <manual>Confirm settings.html contains url_for and the new test passes</manual>
  </verify>
  <done>
    - `grep 'url_for' triggarr/templates/settings.html` matches the form action line
    - `grep 'hardcoded.*settings\|action="/settings"' triggarr/templates/settings.html` returns 0 matches
...
All checks passing = tech debt audit is clean.
  ...
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && python -m pytest tests/ -x -q 2>&1 | tail -3</automated>
  </verify>
  <done>
...
<verification>
1. `grep -n 'load_settings' triggarr/web/routes.py` returns 0 matches
2. `grep -n 'mock_load\|mock_new_settings' tests/test_web.py` returns 0 matches
3. `grep 'url_for.*save_settings' triggarr/templates/settings.html` returns a match
