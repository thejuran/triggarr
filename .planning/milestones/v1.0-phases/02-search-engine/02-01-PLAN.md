...
must_haves:
  ...
  artifacts:
    ...
    - path: "triggarr/search/__init__.py"
...
Output: Extended config models, client search methods, and `triggarr/search/engine.py` with filter_monitored, slice_batch, and append_search_log functions.
</objective>
...
<task type="auto">
  ...
  <action>
    **Config model extension (triggarr/models/config.py):**
    ...
    **Default config template (triggarr/config.py):**
    ...
    **RadarrClient.search_movies (triggarr/clients/radarr.py):**
    Add method:
    ```python
    ...
    **SonarrClient.search_season (triggarr/clients/sonarr.py):**
    Add method:
    ```python
...
from triggarr.config import DEFAULT_CONFIG
...
<task type="auto">
  ...
  <action>
    **Create triggarr/search/__init__.py:**
    Empty package marker file (same pattern as clients/__init__.py).

    **Create triggarr/search/engine.py with these functions:**
    ...
    3. `append_search_log(state: TriggarrState, app: str, queue_type: str, item_name: str) -> None` — Append a search log entry to state["search_log"]. Entry format: `{"name": item_name, "timestamp": ISO-8601 UTC string with Z suffix, "app": app, "queue_type": queue_type}`. Cap at 50 entries (SEARCH_LOG_MAX = 50), evicting oldest when exceeded: `state["search_log"] = state["search_log"][-SEARCH_LOG_MAX:]`. Use `datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")` for timestamp.
    ...
    All functions are pure (no side effects except append_search_log which mutates the state dict in place). Import TriggarrState from triggarr.state. Import datetime, timezone from datetime stdlib.
  </action>
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && python -c "
from triggarr.search.engine import filter_monitored, slice_batch, append_search_log, deduplicate_to_seasons, filter_sonarr_episodes
...
<verification>
- `python -c "from triggarr.models.config import ArrConfig; a=ArrConfig(); assert a.search_interval==30"` passes
- `python -c "from triggarr.search.engine import slice_batch; assert slice_batch([1,2,3], 0, 2) == ([1,2], 2)"` passes
...
- triggarr/search/engine.py has filter_monitored, slice_batch, append_search_log, deduplicate_to_seasons, filter_sonarr_episodes
