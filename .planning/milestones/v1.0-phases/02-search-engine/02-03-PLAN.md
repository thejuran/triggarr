...
Output: Running Triggarr application that automatically cycles through wanted items on configurable intervals, with full test coverage of search engine utility functions.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    **Create triggarr/search/scheduler.py:**
    ...
    **Update triggarr/__main__.py:**
    ...
    3. The uvicorn log_level should be "warning" so uvicorn access logs don't clutter the loguru output. Triggarr's own logging goes through loguru.
    4. Import `from triggarr.state import STATE_PATH` for the default state path.
    5. Keep KeyboardInterrupt handling in main().

    **Update triggarr/startup.py:**
...
<task type="auto">
  ...
  <action>
    ...
    **Utility function tests (no mocking):**
    ...
    Use `from triggarr.state import _default_state` for state fixtures. Use `from datetime import datetime, timezone, timedelta` for date manipulation. No external mocking libraries needed for these tests.

    Each test should be a standalone function (not in a class). Use descriptive assertions. Import all functions from `triggarr.search.engine`.
  </action>
  <verify>
    <automated>cd /Users/julianamacbook/triggarr && python -m pytest tests/test_search.py -v --tb=short 2>&1 | tail -30</automated>
  </verify>
...
- `python -c "from triggarr.search.scheduler import create_lifespan"` imports without error
