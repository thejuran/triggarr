...
~10,000 Python LOC with 873 tests passing in the current S06 closure verification run. 8 historical milestones shipped (v1.0–v2.6.1-dev). All 18 tracked requirements are validated, with 2 deferred and 3 explicit anti-features/out-of-scope items.

Current active milestone: Portable Config Directory & Documentation Refresh. S01 verified the portable config/data directory contract: `TRIGGARR_CONFIG_DIR` must be absolute, controls `triggarr.toml`, `state.json`, and derived `triggarr.db` when set before process import/startup, and `/config` remains the env-unset Docker/default fallback. S02 refreshed README/TODO/SECURITY-facing documentation for nested multi-instance config, standalone config directories, Docker behavior, and auth/security posture. S03 completed integrated verification evidence for the original documentation refresh, but validation found auth/proxy documentation and evidence-artifact gaps. S04 remediated those gaps: session-cookie Secure decisions now rely on ASGI `request.url.scheme`, Uvicorn remains the sole forwarded-proto trust boundary through `TRUSTED_PROXY_IPS`, README/SECURITY External-auth guidance requires upstream authentication/authorization plus blocked direct access, docs-accuracy tests guard those claims, and S04 evidence supersedes the stale S02 placeholder summary. S05 added the documentation-UAT/release-gate evidence trail: a cold-reader review packet for `README.md`, `SECURITY.md`, and `TODO.md`, an explicit unresolved human UAT/deep-review gate note for auto-mode, and a release-gate evidence index with passing mechanical diagnostics. S06 closed validation-remediation bookkeeping by adding an explicit requirement-scope artifact, canonical S02 supersession instructions, a truthful human UAT/deep-review gate artifact, and a fresh S06 validation evidence index with mechanical checks passing while preserving needs-attention for the unresolved human gate.

Release-readiness caveat after S06: mechanical verification passes, but human documentation UAT and the `/deep-review` release decision remain unresolved because auto-mode had no human approval, change request, or explicit deferral. Milestone validation should remain needs-attention until a human resolves that gate. Existing Starlette TestClient per-request cookie deprecation warnings are still present in auth tests but do not fail verification.
...
- **Evidence/Gates:** S06 artifacts separate mechanical validation readiness from human documentation UAT/release approval. `S06-REQUIREMENT-SCOPE.md` scopes M001 requirement validation, `S06-S02-SUPERSESSION.md` repairs the S02 evidence chain, `S06-HUMAN-UAT-GATE.md` is the source for unresolved human gate status, and `S06-VALIDATION-EVIDENCE.md` indexes mechanical diagnostics and the needs-attention validation recommendation.
...
- [ ] M001: Portable Config Directory & Documentation Refresh — S01/S02/S03/S04/S05/S06 complete; final milestone validation still needs human docs UAT and `/deep-review` approval, change request, completed review, or explicit human deferral before a pass posture
